...
local function predictXYZ(vx, vy, vz, mx, my, mz, yaw, yawRate, dt)
    local vel_x_dt          = vx * dt
    local vel_z_dt          = vz * dt
    local mom_x_dt_sq       = mx * dt * dt
    local mom_x_dt_cb       = mx * dt * dt * dt
    local mom_z_dt_cb       = mz * dt * dt * dt
    local yaw_rate_dt       = yawRate * dt
    local yaw_rate_sq_dt_sq = yawRate * yawRate * dt * dt

    local vel_mag    = math.sqrt(vx*vx + vy*vy + vz*vz)
    local vel_mag_dt = vel_mag * dt
    local look_x = -math.sin(yaw)
    local look_z = -math.cos(yaw)

    local speed_strafe    = vx * (-look_z) + vz * look_x
    local speed_strafe_dt = speed_strafe * dt
    local mom_fwd         = mx * look_x + mz * look_z
    local mom_fwd_dt_sq   = mom_fwd * dt * dt

    local disp_x_s1 = vel_x_dt
    local disp_x_s2 = ((vel_z_dt * yaw_rate_dt) - (mom_x_dt_sq)^3) * 0.38470286
    local disp_x_s3 = mom_x_dt_sq / (0.027309248 - (vel_mag_dt * -1.5240344)^3)
    local dx = disp_x_s1 + disp_x_s2 + disp_x_s3

    local disp_z_s1 = vel_z_dt
    local disp_z_s2 = (yawRate * (yaw_rate_sq_dt_sq + -3.0122473))
                    * (mom_x_dt_cb - ((speed_strafe_dt * mom_z_dt_cb) / -2.8258905))
    local disp_z_s3 = -((mom_fwd_dt_sq)^2 * mom_z_dt_cb)
    local dz = disp_z_s1 + disp_z_s2 + disp_z_s3
...
local function updatePlayer(name, player)
    ...
    local dx, _, dz = predictXYZ(
        snapshot.vx, snapshot.vy, snapshot.vz,
        snapshot.mx, snapshot.my, snapshot.mz,
        snapshot.yaw, snapshot.yawRate, dt
    )

    local predX = snapshot.px + dx
    local predY = snapshot.py      -- Y prediction disabled
    ...
    -- Horizontal error only (Y ignored)
    local eX = curPos.X - predX
    local eZ = curPos.Z - predZ
    local tErr  = math.clamp((math.sqrt(eX*eX + eZ*eZ) - 0.5) / 4, 0, 1)
    ...
    if now - lastDebugPrint > 3 then
        lastDebugPrint = now
        print(string.format("[OrbVis] %s | dt=%.0fms | hErr=%.2f",
            name, dt_ms, math.sqrt(eX*eX + eZ*eZ)))
    end
end
...
print("=== Predicted Position Visualizer v3.0 (X/Z boost, Y disabled) ===")
print("Green = accurate horizontal | Red = off")
